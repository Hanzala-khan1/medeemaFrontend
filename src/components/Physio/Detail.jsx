...
const Detail = () => {
  ...
  useEffect(() => {
    ...
    const fetchphysioData = async () => {
      try {
        const docRef = doc(db, "physio", id);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setphysioData(docSnap.data());
        } else {
        }
      } catch (error) {
  ...
  const handleRateChange = (event) => {
    if (physioData) {
      if (event.target.value == "perDay") {
        setFees(parseInt(physioData?.perDay));
      } else {
        setFees(parseInt(physioData?.perHour));
      }
    }
  ...
  const handleDateChange = (event) => {
    ...
    if (formData.rate === "perHour") {
      const amount =
        Number(physioData?.perHour * diffDays * 100) * 8 || physioData?.perHour;
      setAmount(parseInt(amount));
      localStorage?.setItem("amount", amount);
      setFormData((prev) => ({
        ...prev,
        amount: physioData?.perHour * diffDays * 8,
      }));
    } else {
      const amount =
        Number(physioData.perDay * diffDays) * 100 || physioData?.perHour;
      setAmount(parseInt(amount));
      localStorage?.setItem("amount", amount);
      setFormData((prev) => ({
        ...prev,
        amount: physioData?.perDay * diffDays,
      }));
    }
  ...
  const isDateDisabled = (date) => {
    // return physioData?.unavailability.includes(date);
    if (physioData?.availability === "weekdays") {
      const selectedDay = new Date(date).getDay();
      return (
        physioData?.unavailability?.includes(date) ||
        selectedDay === 0 ||
        selectedDay === 6
      );
    }
    if (physioData?.availability === "weekend") {
      const selectedDay = new Date(date).getDay();
      return (
        physioData?.unavailability.includes(date) ||
  ...
  const addData = async () => {
    ...
    if (isLogined) {
      ...
      // ) {
      const physioRef = collection(db, "orders");
      try {
        const res = await addDoc(physioRef, {
          receiverId: physioData?.auth_id,
          // amount: Number(fees * diffDays),
          receiverName: physioData?.name,
  ...
  const calculateBusinessDays = () => {
    ...
    while (currentDate <= endDate) {
      ...
      if (
        dayOfWeek !== 6 &&
        dayOfWeek !== 0 &&
        !physioData?.unavailability?.includes(
  ...
  const calculation = useMemo(() => {
    if (formData.rate === "perHour") {
      return Number(
        physioData?.perHour * diffDays * 8 * 100 -
          (physioData?.discount / 100) *
            (physioData?.perHour * diffDays * 8 * 100)
      );

      // localStorage?.setItem("amount", physioData?.perHour*diffDays *8);
    } else {
      // localStorage?.setItem("amount", physioData?.perDay * diffDays );

      return Number(
        physioData?.perDay * diffDays * 100 -
          (physioData?.discount / 100) * (physioData?.perDay * diffDays * 100)
      );
    }
    return parseInt(physioData?.perDay * diffDays * 100);
  ...
  return (
    <div className="mb-12">
      <Banner
        title={""}
        subHeading1={"Home"}
        subHeading2={"Physio care Details"}
      />
            ...
            {physioData ? (
              <div className="flex gap-2">
                <div>
                  <Image
                    src={
                      physioData?.images?.url ? physioData?.images?.url : noimg
                    }
                    width={100}
                    height={100}
                    alt={physioData?.images?.name}
                ...
                <div className="">
                  ...
                  <div>
                    <h3 className="text-sm text-[#1A3578]">
                      Education: {physioData?.education} /
                      {physioData?.experience} Years Experience{" "}
                    </h3>
                  </div>
                  <div>
                    <h3 className="text-xs text-[#B1B1B1]">
                      Working at {physioData?.workingAt} Hospital
                    </h3>
                  </div>
          ...
          <div className=" mt-6 md:mt-8">
              ...
              <span>
                ...
                {physioData
                  ? formData.rate == "perHour"
                    ? Number(physioData?.perHour * diffDays * 8)
                    : Number(physioData?.perDay * diffDays)
                  : "NA"}
              </span>
              <span>Discount : {physioData?.discount}%</span>
              <span>
                Total :{" "}
                {formData.rate == "perHour"
                  ? Number(
                      physioData?.perHour * 8 * diffDays -
                        (physioData?.discount / 100) *
                          (physioData?.perHour * 8 * diffDays)
                    )
                  : Number(
                      physioData?.perDay * diffDays -
                        (physioData?.discount / 100) *
                          (physioData?.perDay * diffDays)
                    )}
              </span>
            ...
            <div className="grid text-black grid-rows-[80px_80px_80px_80px_120px_80px] ">
              ...
              <div className="flex flex-col px-2 row-span-1 md:col-span-1 col-span-2">
                ...
                >
                  <option value="perHour">{`Per Hour ${physioData?.perHour}`}</option>
                  <option value="perDay">{`Per Day ${physioData?.perDay}`}</option>
                </select>
              </div>
              ...
              <div className="flex flex-col row-span-1 -mt-10 md:-mt-1 px-2 md:col-span-1 col-span-2">
                ...
                <select
                  onChange={handleFormChange}
                  name="patientGender"
                  className="h-[42px] text-lg px-2 mt-1 bg-transparent border border-black rounded-md"
                >
              ...
              <div className="flex flex-col px-2 mt-2 md:-mt-6 col-span-2">
                ...
                >
                  ...
                  <option value="urgent">For Urgent Care</option>
                  <option value="monthly">For Monthly Checkup</option>
