...
const List = () => {
  ...
  const fetchphysio = async () => {
    var arr = [];
    try {
      const q = query(collection(db, "physio"));
  ...
  fetchphysio();
  const { isLoading, data } = useQuery(["Physio"], fetchphysio, {
    staleTime: 60000,
  });
  if (!isLoading) {
    // console.log(data)
