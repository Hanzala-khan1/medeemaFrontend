...
const Detail = () => {
  ...
  const handleRateChange = (event) => {
    if (ayaData) {
      if (event.target.value == "perDay") {
        setFees(parseInt(ayaData?.perDay));
      } else {
        setFees(parseInt(ayaData?.perHour));
      }
    }
  ...
  const handleDateChange = (event) => {
    ...
    if (formData.rate === "perHour") {
      const amount =
        Number(ayaData?.perHour * diffDays * 100) * 8 || ayaData?.perHour;
      setAmount(parseInt(amount));
      localStorage.setItem("amount", amount);
      setFormData((prev) => ({
        ...prev,
        amount: ayaData?.perHour * diffDays * 8,
      }));
    } else {
      const amount =
        Number(ayaData.perDay * diffDays) * 100 || ayaData?.perHour;
      setAmount(parseInt(amount));
      localStorage.setItem("amount", amount);
      setFormData((prev) => ({ ...prev, amount: ayaData?.perDay * diffDays }));
    }
  ...
  const isDateDisabled = (date) => {
    // return ayaData?.unavailability.includes(date);
    if (ayaData?.availability === "weekdays") {
      const selectedDay = new Date(date).getDay();
      return (
        ayaData?.unavailability?.includes(date) ||
        selectedDay === 0 ||
        selectedDay === 6
      );
    }
    if (ayaData?.availability === "weekend") {
      const selectedDay = new Date(date).getDay();
      return (
        ayaData?.unavailability.includes(date) ||
  ...
  const calculateBusinessDays = () => {
    ...
    while (currentDate <= endDate) {
      ...
      if (
        dayOfWeek !== 6 &&
        dayOfWeek !== 0 &&
        !ayaData?.unavailability?.includes(
  ...
  const calculation = useMemo(() => {
    if (formData.rate === "perHour") {
      return Number(
        ayaData?.perHour * diffDays * 8 * 100 -
          (ayaData?.discount / 100) * (ayaData?.perHour * diffDays * 8 * 100)
      );

      // localStorage.setItem("amount", ayaData?.perHour*diffDays *8);
    } else {
      // localStorage.setItem("amount", ayaData?.perDay * diffDays );

      return Number(
        ayaData?.perDay * diffDays * 100 -
          (ayaData?.discount / 100) * (ayaData?.perDay * diffDays * 100)
      );
    }
    return parseInt(ayaData?.perDay * diffDays * 100);
          ...
          <div className="bg-white py-2 px-2">
            {ayaData ? (
              <div className="flex gap-2">
                <div>
                  <Image
                    src={ayaData?.images?.url}
                    width={100}
                    height={100}
                    alt={ayaData?.images?.name}
                ...
                <div className="">
                  ...
                  <div>
                    <h3 className="text-sm text-[#1A3578]">
                      Education: {ayaData.education} /{ayaData.experience} Years
                      Experience{" "}
                    </h3>
                  </div>
                  <div>
                    <h3 className="text-xs text-[#B1B1B1]">
                      Working at {ayaData.workingAt} Hospital
                    </h3>
                  </div>
          ...
          <div className=" mt-6 md:mt-8">
              ...
              <span>
                ...
                {ayaData
                  ? formData.rate == "perHour"
                    ? Number(ayaData?.perHour * diffDays * 8)
                    : Number(ayaData?.perDay * diffDays)
                  : "NA"}
              </span>
              <span>Discount : {ayaData?.discount}%</span>
              <span>
                Total :{" "}
                {formData.rate == "perHour"
                  ? Number(
                      ayaData?.perHour * 8 * diffDays -
                        (ayaData?.discount / 100) *
                          (ayaData?.perHour * 8 * diffDays)
                    )
                  : Number(
                      ayaData?.perDay * diffDays -
                        (ayaData?.discount / 100) * (ayaData?.perDay * diffDays)
                    )}
              </span>
              <span>
                {" "}
            ...
            <div className="grid text-black grid-rows-[80px_80px_80px_80px_120px_80px] ">
              ...
              <div className="flex flex-col px-2 row-span-1 md:col-span-1 col-span-2">
                ...
                >
                  <option value="perHour">{`Per Hour ${ayaData?.perHour}`}</option>
                  <option value="perDay">{`Per Day ${ayaData?.perDay}`}</option>
                </select>
              </div>
